...
namespace Game
{
    public class ScriptConfigLoader : IConfigLoaderRaw
    {
        public System.Collections.IEnumerator _loadRaw(File file) {
            Debug.Log("start load: " + file.GetPath() + " " + Time.time);

            // get file content
            var len = file.GetLen();
            yield return new WaitForFrame(1);
            var buff = file.GetBuffer((int)len);
            Debug.Log("read file : " + len + " " + Time.time);

            // get zip
            // cost much time to uncompress !!!
            int totalCount = 0;
            var steam = new System.IO.MemoryStream(buff);
            ...
            Debug.Log("zip file count : " + totalCount + " " + Time.time);
            Debug.Log("finish load: " + file.GetPath() + " " + Time.time);
        }
    }
}