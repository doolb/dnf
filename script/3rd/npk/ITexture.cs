...
namespace ExtractorSharp.Core
{
    ...
    public class BitmapTexture : ITexture
    {
        ...
        public BitmapTexture(Bitmap img, Size size)
        {
            this.Image = img;
            this.Size = size;
        }
    ...
    public class BitmapTextureUltis : TextureUitls
    {
        private ITexture empty;
        public override ITexture Empty => empty ?? (empty = new BitmapTexture());

        public override ITexture FromArray(byte[] data, Size size, ColorBits bits)
        {
            var bmp = Bitmaps.FromArray(data, size, bits);
            var tex = new BitmapTexture(bmp, size);
            return tex;
        }

        public override ITexture FromArray(byte[] data, Size size)
        {
            var bmp = Bitmaps.FromArray(data, size);
            var tex = new BitmapTexture(bmp, size);
            return tex;
        }
    }
